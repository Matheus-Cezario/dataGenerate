...
func generateName(parans ...string) string{
	...
	switch strings.ToUpper(gender) {
		case "M":
			content, err := os.ReadFile("generate/names/masc.txt")
			if err != nil {
				return "File Masc not found!"
			}
			listName := strings.Split(string(content),"\n")
			randomIndex := rand.Intn(len(listName))
    		return strings.ReplaceAll(listName[randomIndex],"\r","")
		case "F":
			content, err := os.ReadFile("generate/names/fem.txt")
			if err != nil {
				return "File Fem not found!"
			}
			listName := strings.Split(string(content),"\n")
			randomIndex := rand.Intn(len(listName))
    		return strings.ReplaceAll(listName[randomIndex],"\r","")
		default:
			var file = "generate/names/masc.txt"
			if rand.Float32() > 0.5 {
				file = "generate/names/fem.txt"
			}
